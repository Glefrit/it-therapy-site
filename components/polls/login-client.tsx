...
export default function LoginClient() {
  ...
  useEffect(() => {
    try {
      ...
      if (!saved) return;
      pollFetch("/api/admin/session", { headers: { authorization: `Bearer ${saved}` } })
  ...
  async function signIn(event: React.FormEvent) {
    ...
    try {
      const res = await pollFetch("/api/admin/session", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ password }) });
  ...
  async function signOut() {
    const res = await pollFetch("/api/admin/session", { method: "DELETE", headers: { authorization: `Bearer ${token}` } }).catch(() => null);
